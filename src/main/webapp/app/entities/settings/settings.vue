  ...
  <div class="row justify-content-center">
    <div class="col-8">
            ...
            <div class="card-body">
              <div class="row">
                <div class="col-md-4 form-group mb-3">
                  <label class="form-control-label font-sm text-muted mb-1" v-text="t$('stackApp.settings.fileUpload.defaults.defaultMaxFileSizeBytes')"></label>
                ...
                <div class="col-md-4 form-group mb-3">
                  <label class="form-control-label font-sm text-muted mb-1" v-text="t$('stackApp.settings.fileUpload.defaults.defaultMaxRequestSizeBytes')"></label>
                  <div class="input-group">
                    <input
                      type="number"
                      class="form-control"
                ...
                <div class="col-md-4 form-group mb-3 d-flex align-items-end pb-2">
                  <div class="form-check form-switch ps-5">
                    ...
                    />
                    <label class="form-check-label ms-2" for="blockUnmatched" v-text="t$('stackApp.settings.fileUpload.defaults.blockUnmatched')"></label>
                  </div>
                </div>
              </div>
              <div class="form-group mb-0 mt-2">
                <label class="form-control-label font-sm text-muted mb-1" v-text="t$('stackApp.settings.fileUpload.defaults.welcomeMessage')"></label>
                <textarea class="form-control border-light" rows="2" v-model="settings.fileUploadDefaults.welcomeMessage" placeholder="업로드 화면에 표시될 안내 문구를 입력하세요."></textarea>
                <small class="text-info mt-1 d-block"><i class="bi bi-info-circle me-1"></i> <span v-text="t$('stackApp.settings.fileUpload.defaults.blockUnmatchedHelp')"></span></small>
              </div>
            </div>
          </div>
                ...
                <table class="table table-hover mb-0 align-middle">
                  ...
                  <tbody>
                    <tr v-for="(policy, index) in settings.fileTypePolicies" :key="index" class="border-bottom">
                      <td class="ps-4">
                        <input type="text" class="form-control form-control-sm mb-1 fw-bold" v-model="policy.label" :placeholder="t$('stackApp.settings.fileUpload.policy.label')" />
                        <input type="text" class="form-control form-control-sm text-secondary bg-light border-0" v-model="policy.key" :placeholder="t$('stackApp.settings.fileUpload.policy.key')" style="font-size: 0.7rem;" />
                      </td>
                      <td>
                        <textarea
                      ...
                      <td class="pe-4 text-end">
                        <div class="d-flex align-items-center">
                          <b-dropdown size="sm" variant="light" class="shadow-sm border rounded me-1" right no-caret>
                            ...
                            <h6 class="dropdown-header text-uppercase font-xs text-muted fw-bold mb-1" v-text="t$('stackApp.settings.fileUpload.policy.template')"></h6>
                            <b-dropdown-item v-for="tpl in settings.fileTypeTemplates" :key="'row-tpl-' + tpl.key" @click.prevent="applyTemplate(index, tpl)">
                              <span v-text="tpl.label"></span>
                            </b-dropdown-item>
                            <b-dropdown-item v-if="!settings.fileTypeTemplates || settings.fileTypeTemplates.length === 0" disabled>
                              등록된 템플릿 없음
                  ...
                  <table class="table table-sm table-borderless font-sm mb-0">
                    ...
                    <tbody>
                      <tr v-for="guide in getTemplateGuide()" :key="guide.key">
                        ...
                        <td><code>{{ getCommaSeparated(guide.allowedExtensions) }}</code></td>
                        <td><code class="text-break">{{ getCommaSeparated(guide.allowedMimeTypes) }}</code></td>
                      </tr>
                      <tr>
            ...
            <table class="table table-hover align-middle mb-0">
              ...
              <tbody>
                <tr v-for="(tpl, index) in settings.fileTypeTemplates" :key="'tpl-' + index">
                  <td class="ps-4">
                    <input type="text" class="form-control form-control-sm border-0 bg-light rounded" v-model="tpl.label" placeholder="템플릿 이름" />
                  </td>
                  <td>
                    <input type="text" class="form-control form-control-sm border-0 bg-light rounded" :value="getCommaSeparated(tpl.allowedExtensions)" 
                           @input="updateCommaSeparated(tpl, 'allowedExtensions', $event)" placeholder="jpg, png..." />
                  </td>
                  <td>
                    <textarea class="form-control form-control-sm border-0 bg-light rounded" rows="1" :value="getCommaSeparated(tpl.allowedMimeTypes)"
                              @input="updateCommaSeparated(tpl, 'allowedMimeTypes', $event)" placeholder="image/jpeg, image/png..."></textarea>
                  </td>
                  <td>
                    <div class="input-group input-group-sm">
                      <input type="number" class="form-control border-0 bg-light" :value="getMB(tpl.maxFileSizeBytes)" @input="updatePolicyMB(tpl, $event)" />
      ...
      <div class="d-flex justify-content-end gap-2 mb-5 px-2">
        ...
        <button type="submit" id="save-entity" data-cy="entityCreateSaveButton" :disabled="isSaving" class="btn btn-primary rounded-pill px-5 py-2 shadow border-0 gradient-primary">
